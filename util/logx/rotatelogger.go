...
const (
	...
	fullFormat      = "2006-01-02_15-04-05"
	hoursPerDay     = 24
	bufferSize      = 100
...
type (
	...
	RotateRule interface {
		...
		MarkRotated(size int64)
		OutdatedFiles() []string
		ShallRotate(size int) bool
	}
...
func (r *DailyRotateRule) MarkRotated(size int64) {
...
func (r *DailyRotateRule) ShallRotate(size int) bool {
	ok := (len(r.rotatedTime) > 0 && getNowDate() != r.rotatedTime) || (r.rotatedSpace + int64(size)) > r.maxSpace
	if !ok {
		r.rotatedSpace += int64(size)
	}
	return ok
}
...
func (l *RotateLogger) write(v []byte) {
	if l.rule.ShallRotate(len(v)) {
		if err := l.rotate(); err != nil {
			log.Println(err)
		} else {
			l.rule.MarkRotated(0)
		}
	}
	if l.fp != nil {
		l.fp.Write(v)
...
func getNowTime() string {
	return time.Now().Format(fullFormat)
}
