...
func setupLogLevel(c logx.LogConf) {
	switch c.Level {
	case levelInfo:
		SetLevel(InfoLevel)
	case levelError:
...
func setupWithConsole(c logx.LogConf) {
	once.Do(func() {
		atomic.StoreUint32(&initialized, 1)
		writeConsole = true
		setupLogLevel(c)
...
func setupWithFiles(c logx.LogConf) error {
	var opts []LogOption
	var err error
...
func setupWithVolume(c logx.LogConf) error {
